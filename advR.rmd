...
# when the logical in if() is not a typical one it interprets 0 as FALSE and any other number as TRUE
...
```{r}
# for loops with next and break
# next will exit the current iteration
# break will exit the entire loop

# so in this example we wont print anything less than 3 and we'll stop when we get to an index of 5 or greater
for (i in 1:10) {
  if (i < 3) 
    next
...
means <- c(1, 50, 20)
out <- vector("list", length(means)) # this is the allocated output container
for (i in 1:length(means)) {
  out[[i]] <- rnorm(10, means[[i]])
}
out # a list with 3 elements in it and each element contains 10 random numbers


# next don't iterate over length(x), this has some buggy issues when length(x) is 0, instead use seq_along(x)

out <- vector("list", length(means))
for (i in seq_along(means)) {
  out[[i]] <- rnorm(10, means[[i]])
}
out # even though we may not see a difference, by using seq_along we're avoiding errors when means has length 0
...
# It’s possible (although typically not useful) to call an anonymous function. Which of the two approaches below
# is correct? Why?

# function(x) 3() # invalid
#> function(x) 3()
(function(x) 3)() # valid, this function function(x) 3 is wrapped in parenthesis before being called by ()
# the wrapped parenthesis are what makes the anonymous function work
#> [1] 3
...
```{r}
# a small example to show how it works, basically values defined within a function are used first
x <- 10
g01 <- function() {
  x <- 20
  x
...
g04() # 1 2 3, this shows off the order in which R looks for variables, first in current function, then where 
# function is defined, and so on up to the global environment
```
...
g08() # the first function named g07 does not get utilized since R found a function named g07 in the current 
# function
#> [1] 110

# since functions are in a way objects, they work the same way in terms of lexical scoping
...
# on the whole its best to not give objects and functions the same name because it can get complicated
...
# fresh start!! it seems like objects that are bound to a name in a function to do not become part of the
# environment when the function is run, this was seen earlier too as the value of y defined outside of the function stayed the same after a function was run which bound another object to the name y.
...
#> [1] 10, this works because x is never accessed
...
f2() # 100, this shows off lazy evaluation as x = z gets lazily evaluated after z has already been bound to 100
```

```{r}
y <- 10
f1 <- function(x = {y <- 1; 2}, y = 0) { # {y <- 1;2} assigns 1 to y and then outputs 2 which gets assigned to x
  c(x, y)
}
f1() # 2 1 
# When y gets accessed next within c(), it has already the value 1 and R doesn’t need to look it up any further.
# Therefore, the promise y = 0 won’t be evaluated.
y # 10, also the global y value is left untouched
...
show_time() 
#> [1] "2021-01-16 14:30:34 PST"

# before x = stop("Error!") is accessed the stop function gets overwritten to be a function that outputs
# Sys.time() so when x is called it is evaluated as Sys.time()
```
...
i02(x = 1, y = 2, z = 3) # creates a list with elements 2 and 3. x is unused here because it isn't called
...
a <- 1
(a <- 1) # outputs 1 because we wrapped the function in parenthesis
...
j05() # Error in j05() : I'm an error

# exit handlers
# you can use on.exit to output something no matter what happens with the function
# you have to include add = TRUE for it to be printed
...
names(df) <- c("x", "y", "z")
`names<-`(df, c("x", "y", "z")) # in prefix form

for(i in 1:10) print(i)
`for`(i, 1:10, print(i)) # in prefix form
...
k01(1, 2, 3) # 1 2 3
k01(2, 3, abcdef = 1) # still returns 1 2 3 since R looks at exact matches first 

# the order is (exact matches, unique prefixes, position)
# unique prefixes doesn't work when the prefixes are ambiguous
```
...
# :, ::, :::, $, @, ^, *, /, +, -, >, >=, <, <=, ==, !=, !, &, &&, |, ||, ~, <-, and <<-
# you can also create your own infix functions by wrapping with %
...
`second<-` <- function(x, value) {
  x[2] <- value
  x
} # in a vector this replaces the second element with a new value

x <- 1:10
second(x) <- 5L 
x #  [1]  1  5  3  4  5  6  7  8  9 10
...
modify(x, 1) <- 10
x  #  [1]  10  5  3  4  5  6  7  8  9 10
# behind the scenes R does: 
x <- `modify<-`(x, 1, 10)

mod <- function(x, position, value){
  x[position] <- value
  x
} # in prefix form

# of course it's easier to write the function in prefix form, but if you wanted to use the function as 
# one that take the form function(...) <- 10 then you can use `function<-` as the name!
...
`randrep<-` <- function(x, value){
  id <- sample(length(x), 1)
  x[id] <- value
  x
}

# Write your own version of + that pastes its inputs together if they are character vectors but behaves as
# usual otherwise. In other words, make this code work:
...
# create an infix xor function
# xor is exclusive or, so either a or b but not a and b
...
# Modify where() to return all environments that contain a binding for name. Carefully think through what 
...
# good places to use messages

# When a default argument requires some non-trivial amount of computation and you want to tell the user what
# value was used. (binwidth with ggplot)

# some other high level reasons

# difference between cat and message
...
# the outputs are similar but the purposes are different
# use cat when the primary role of the function is to print into the console (upper-right), ex: print(), str()
# Use message() as a side-channel to print to the console when the primary purpose of the function is
# something else

# in other words use cat when a user wants something printed and message when the developer _elects_ to print 
# something
```
...
# write a wrapper for file.remove that throws an error if the file to be deleted does not exist
